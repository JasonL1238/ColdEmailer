...
**Find Person** — The one person-first screen. Give a full name plus whatever else you know (ideally the exact LinkedIn member URL, as well as company, school and years, past employers, role, location, or free text). A supplied profile URL becomes an identity constraint: Reach excludes same-name alternatives, uses public snippets for that exact profile to fill company/role gaps, and collects addresses for review. Without a profile URL it scores the available evidence to tell namesakes apart. Nothing reaches your database until you approve a candidate. A pattern-guessed address (`first.last@company.com`) can only be saved as sendable when the address matches the person **and** either a mail server confirmed the mailbox or a public corpus shows that address in use under their name — and you still have to confirm ownership yourself. Otherwise it is recorded as a note. Runs hard-stop at 10 minutes.
